#!/bin/sh
# Control Panel v0.1 - alphp 7.4 web-based user interface.
#
...
case $QUERY_STRING in
    mysql-start*)
        if [ ! -d "$MYSQL_DATA_DIR/mysql" ]; then
            mysql_install_db --datadir="$MYSQL_DATA_DIR" --auth-root-authentication-method=normal > /dev/null 2>&1
            mysql_start
...
<html lang="id">
    <head>
        ...
        <title>alphp 7.4 Control Panel</title>
        <style>
